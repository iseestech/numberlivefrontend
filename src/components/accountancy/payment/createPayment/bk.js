...
class CreatePayment extends React.Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'sale_desc',
          responsive: ['xs', 'sm'],
          render: (key, record) => <span>122</span>,
        },
        {
          title: 'Invoice Amount',
          dataIndex: 'quantity',
          responsive: ['xs', 'sm'],
          render: (key, record) => <span>12200</span>,
        },
        {
          title: 'Amount Due',
          dataIndex: 'sale_unit_price',
          responsive: ['xs', 'sm'],
          render: (key, record) => <span>14/02/2021</span>,
        },
        {
          title: 'Payment',
          dataIndex: 'sale_discount',
          responsive: ['xs', 'sm'],
          render: (key, record) => <span>1000</span>,
        },
        // {
        //   title: 'Account',
        //   responsive: ['xs', 'sm'],
  ...
  componentDidMount = () => {
    ...
    if (match.params.invoiceId) {
      // console.log(match.params.invoiceId, '(match.params.invoiceId')
      const fetchInvoiceData = async () => {
        const result = await QuotationService.getInvoice(match.params.invoiceId, 'post')
  ...
  onFinish = values => {
    ...
    if (match.params.invoiceId) {
      method = 'put'
      values.id = match.params.invoiceId
    }
    values.email = user.email
    // console.log(values, 'values')
    const fetchData = async () => {
      const result = await QuotationService.createInvoice(values, method)
  ...
  handleChange = (e, record, fieldName) => {
    ...
    dataSource.forEach(element => {
      if (element.id === record.id) {
        ...
        const { sale_unit_price: unitPrice, quantity } = record
        ...
        if (name === 'sale_discount') {
          ...
          const currentField = ['quantity', 'sale_unit_price']
      ...
      <>
        ...
        >
          <div className="row">
            ...
            <div className="col-md-2">
              <Form.Item name="invoiceNo" label="Amount Received">
                <Input disabled />
              </Form.Item>
            </div>
            <div className="col-md-2">
              <Form.Item name="invoiceNo" label="Payment Number#">
                <Input disabled />
              </Form.Item>
            </div>
            <div className="col-md-2">
            ...
            <div className="col-md-3">
              <Form.Item name="piNumber" label="Deposit To">
                <Input />
              </Form.Item>
            </div>
            <div className="col-md-3">
