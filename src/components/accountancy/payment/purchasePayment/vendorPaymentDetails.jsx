...
class VendorPaymentDetails extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentDidMount = () => {
    ...
    if (match.params.id) {
      ...
      const fetchInvoiceData = async () => {
        const result = await VendorAdvPaymentService.getVendorAdvDetails(match.params.id)
  ...
  applyToBills = async () => {
    ...
    const { match } = this.props
    const { paymentMaserData } = this.state

    // console.log('apply to bills')
    const result = await PurchaseService.getUnpaidInvoiceNew({
      id: paymentMaserData.vendorId,
      orgCode: org.orgCode,
    })
  ...
  render() {
    ...
    const menu = (
      <Menu>
        ...
        <Menu.Item>
          <Link to={`/purchase/vendorPayment/edit/${match.params.id}`}>Edit</Link>
        </Menu.Item>
      ...
      <div className="details_layout--page">
        ...
        {isModalVisible && (
          <Modal
            title={
              <div>
                <h6 className="ant-modal-title">UNPAID INVOICES</h6>Balance:{amountPaid}
              </div>
            }
          ...
          >
            ...
            <ApplyToBillsModal vendorId={vendorId} receivedAmount={amountPaid} />
          </Modal>
        )}
      </div>
    )
...
export default connect(mapStateToProps)(VendorPaymentDetails)
