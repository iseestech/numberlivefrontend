...
class CreateQuotation extends React.Component {
  ...
  componentDidMount = () => {
    ...
    //     this.setState({ prodData: prodResult.data }, () => this.fetchCustData())
    //   }
    // }
    ...
    if (params.quoteId) {
      const fetchInvoiceData = async () => {
        const result = await QuotationService.getQuotation(params.quoteId, 'get')
  ...
  onFinish = values => {
    ...
    if (params.quoteId) {
      method = 'put'
      // values.id = params.quoteId
      newData.sale_order_no = params.quoteId
    }
    ...
    const fetchData = async () => {
      const result = await QuotationService.createQuotation(finalData, method)
      this.setState(
        {
          isDirty: false,
        },
        () => this.props.navigate('/sales/quotations'),
      )
    }
  ...
  handleChange = (e = 0, record = {}, fieldName = '') => {
    ...
    const { dataSource, taxValue, prodData, taxData, discountNew, shippingCharges } = this.state
    let amount = 0
    let subAmount = 0
    let totalTax = 0
    let totalDiscount = 0
  ...
  updateProduct = async (e, row, keyName, index) => {
    ...
    if (keyName === 'name') {
      ...
      // console.log(returnedTarget, 'dataSource123', dataSource)
      this.formRef.current.setFieldsValue({ productDto: dataSource })
    ...
    return (
      <>
        <Modal
          ...
          width={1000}
        >
          <CreateCustomer fromModal handleNewCustomer={this.handleNewCustomer} />
        </Modal>
        <Form
            ...
            <div className="col-md-6" style={{ display: isShown ? 'block' : 'none' }}>
              ...
              <Col span={24}>
                <Form.List name="productDto">
                  {(productDto, options) => {
                    // console.log(dataSource, 'dataSource.productDto', prodData)
                    return (
                      <ProductTable
...
export default connect(mapStateToProps)(WithRouter(CreateQuotation))
