...
const VendorAdvPaymentService = {
  getAllVendorAdvPayment: value => {
    return apiClient
      .get('/isees/api/vendoradvanced/all')
      .then(response => {
        return response.data && response.data
      })
      .catch(err => console.log(err))
  },
  addAdvVendorPayment: (values, method) => {
    const qs = queryString.stringify(values)
    console.log(values, 'from services')
    let url = 'isees/api/vendoradvanced/vendoradvanced'
    if (method === 'put') {
      url = 'isees/api/vendoradvanced/update'
    }
  ...
  getVendorAdvDetails: id => {
    return apiClient
      .get(`/isees/api/vendoradvanced/id?paymentNo=${id}`)
      .then(response => {
        return response.data && response.data
      })
      .catch(err => console.log(err))
  },
  getVendorInvoices: (id, orgCode) => {
    return apiClient
      .get(`/isees/api/vendoradvanced/getallinvoicewithvendor?paymentNo=${id}&orgCode=${orgCode}`)
      .then(response => {
        return response.data && response.data
      })
      .catch(err => console.log(err))
  },
  addVendorAdvInvoice: (values, method) => {
    const qs = queryString.stringify(values)
    console.log(values, 'from services')
    let url = 'isees/api/vendoradvanced/vendoradvancedinvoice'
    if (method === 'put') {
      url = 'isees/api/vendoradvanced/update'
    }
    return apiClient[method](url, values)
  ...
  getVendorPaymentData: id => {
    return apiClient
      .get(`/isees/api/vendoradvanced/data?paymentNo=${id}`)
      .then(response => {
        return response.data && response.data
      })
      .catch(err => console.log(err))
...
export default VendorAdvPaymentService
